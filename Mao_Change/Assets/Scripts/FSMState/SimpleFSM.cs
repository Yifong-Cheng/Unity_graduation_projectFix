...
public class SimpleFSM : FSM 
{
	...
	protected override void Initialize()
	{
		...
		controller = role.GetComponent<CharacterController>();
		//Get animation
		animComponent = role.GetComponent<Animation>();
		...
		if (!playerTransform)
			Debug.Log ("Player doesn't exist, please add one player with Tag 'Player'");
	...
	protected void UpdatePatrolState()
	{
		...
		if (Vector3.Distance(role.transform.position, destPos) <= arriveDistance)
		{
			Debug.Log ("Reached to the destination point, calculating the next point");
			FindNextPoint();
		}
		else if (Vector3.Distance(role.transform.position, playerTransform.position) <= chaseDistance)
		{
		...
		Quaternion targetRotation = Quaternion.LookRotation(destPos - role.transform.position);
        role.transform.rotation = Quaternion.Slerp(role.transform.rotation, targetRotation, Time.deltaTime*curRotSpeed);

		//Go Forward
		controller.SimpleMove(role.transform.forward * Time.deltaTime * walkSpeed);
	...
	protected void UpdateChaseState()
	{
		...
		float dist = Vector3.Distance(role.transform.position, playerTransform.position);
		...
		//Go Forward
		controller.SimpleMove(role.transform.forward * Time.deltaTime * runSpeed);
	...
	protected void UpdateAttackState()
	{
		...
		targetRotation = Quaternion.LookRotation(destPos - role.transform.position);
        role.transform.rotation = Quaternion.Slerp(role.transform.rotation, targetRotation, Time.deltaTime*curRotSpeed);
